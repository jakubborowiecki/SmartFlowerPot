	...
	for(uint8_t i=0; i<font[1]; i++ ) // Each column (Width)
	{
        ...
        for(int8_t j=0; j<font[0]; j++, line >>= 1) // For each pixel in column
        {
            if(line & 1) // Check last pixel in line
            {
            	if(size == 1)
            	{
            	drawPixel(x+i, y+j, color); // Draw this pixel
            	}
            	else
            		writeFillRect(x+i*size, y+j*size, size, size, color); // Or bigger pixel
            }
            else if(background == 0)
            {
            	if(size == 1)
            	{
            		drawPixel(x+i, y+j, background); // Draw black BG
            	}
				else
					writeFillRect(x+i*size, y+j*size, size, size, background); // Or bigger
            }
        }
    }
}
...
void DrawString(int x, int y, char* str, uint8_t color, uint8_t background)
{
	...
	while(*str++)
	{
		DrawChar(x_tmp, y, znak, color, background); // Draw current char
